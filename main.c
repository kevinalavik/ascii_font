...
int main(int argc, char* argv[])
{
    ...
    for (size_t ch = 0; ch < ASCII_MAX; ch++)
    {
        ...
        for (ptrdiff_t y = 0; y < face->glyph->bitmap.rows; y++)
        {
            ...
            {
                ptrdiff_t row = ch / ROW_WIDTH * height + top + y;
                ptrdiff_t col = ch % ROW_WIDTH * width + left + x;
                bitmap[row * ROW_WIDTH * width + col] = face->glyph->bitmap.buffer[y * face->glyph->bitmap.width + x];
            }
        }
    }

    FILE* preview = fopen("preview.ppm", "wb");
    fprintf(preview, "P5\n%zu %zu 255\n", width * ROW_WIDTH, height * ASCII_MAX/ROW_WIDTH);
    fwrite(bitmap, size * ASCII_MAX, 1, preview);
    fclose(preview);

    FILE* header = fopen("ascii_font.h", "wb");
    fprintf(header, "// Generated by: https://github.com/hubenchang0515/ascii_font\n");
    fprintf(header, "// Command: %s %s %s\n", argv[0], argv[1], argv[2]);
    ...
    for (size_t ch = 0; ch < ASCII_MAX; ch++)
    {
        ...
        for (size_t y = 0; y < height; y++)
        {
            ...
            {
                ptrdiff_t row = ch / ROW_WIDTH * height + y;
                ptrdiff_t col = ch % ROW_WIDTH * width + x;
                fprintf(header, "%u, ", (unsigned) bitmap[row * ROW_WIDTH * width + col]);
            }
            fprintf(header, "},\n");
        }
        fprintf(header, "        },\n");
